...
by EightBitz
Version 0.95b
2017-02-01, 06:00 AM CST

RWExport-To-HTML.ps1 transforms a Realm Works export file into a formatted HTML file. The formatted HTML file relies on a "RWExport_091b_095b.css" file for formatting information.
...
IMPORTANT:
Make sure that the HTML file and the RWExport_091b_095b.css file are in the same directory, otherwise, the HTML file will have no formatting.
...
By default, the HTML output file looks for "RWExport_091b_095b.css" to define its style. You can use this option to specify a different name.
...
For now, at least, you will have to manually copy RWExport_091b_095b.css or some other css file you like and make whatever changes you wish.
...
.OUTPUTS
An HTML file that uses an external style sheet for formatting. The external style sheet should be named "RWExport_091b_095b.css" and should be in the same folder as the HTML output. The RWExport_091b_095b.css file will not be generated by this script, but should already exist.
.EXAMPLE 
...
As a side note, you can set up a separate folder for each export, and put a different RWExport_091b_095b.css file in each folder, so you can give each export a completely different look (by editing the RWExport_091b_095b.css file in each folder).



AND SPEAKING OF RWExport_091b_095b.css:

This script and its resulting HTML file assume the existence of a file named "RWExport_091b_095b.css". If this file is not in the same folder as the resulting HTML file, it will not display correctly.
...
param (
    ...
    [Parameter()] 
    [string]$CSSFileName = "RWExport_091b_095b.css",
...
# Main {
   ...
   if ($Log) {
      Try {
        ...
        [System.IO.File]::AppendAllText($Log,"Version 0.95b`r`n")
        [System.IO.File]::AppendAllText($Log,"2017-02-01, 06:00 AM CST`r`n`r`n")
