...
RWExport-To-HTML.ps1 transforms a Realm Works export file into a formatted HTML file. The formatted HTML file relies on a "RWExport_09a.css" file for formatting information.
...
IMPORTANT:
Make sure that the HTML file and the RWExport_09a.css file are in the same directory, otherwise, the HTML file will have no formatting.
...
By default, the HTML output file looks for "RWExport_09a.css" to define its style. You can use this option to specify a different name.
...
For now, at least, you will have to manually copy RWExport_09a.css or some other css file you like and make whatever changes you wish.
...
.OUTPUTS
An HTML file that uses an external style sheet for formatting. The external style sheet should be named "RWExport_09a.css" and should be in the same folder as the HTML output. The RWExport_09a.css file will not be generated by this script, but should already exist.
.EXAMPLE 
...
As a side note, you can set up a separate folder for each export, and put a different RWExport_09a.css file in each folder, so you can give each export a completely different look (by editing the RWExport_09a.css file in each folder).



AND SPEAKING OF RWExport_09a.css:

This script and its resulting HTML file assume the existence of a file named "RWExport_09a.css". If this file is not in the same folder as the resulting HTML file, it will not display correctly.
...
param (
    ...
    [Parameter()] 
    [string]$CSSFileName = "RWExport_09a.css",
...
Function ParseTopic($PassedTopic,$Outputfile,$Sort,$Prefix,$Suffix,$Details,$Indent,$SeparateSnippets,$InlineStats,$SimpleImageScale,$SmartImageScale,$Parent,$TitleCSS,$TopicCSS,$TopicDetailsCSS,$SectionCSS,$SnippetCSS,$Indcrement,$KeepStyles,$SplitTopics,$Log,$CSSFileName,$CommandLine) {
   ...
   if ($SplitTopics) {
      ...
      $DupeName = 1
      While (Test-Path $HTMLFile) {
         $HTMLFile = $OutputFile + $TopicFile + "-" + $DupeName.ToString() + ".html"
         $DupeName++
      }
      $TopicFile = $HTMLFile
   ...
   If ($PassedTopic.alias) {
      $Aliases = ParseAliases $PassedTopic $TopicFile $Log
   ...
   If ($PassedTopic.linkage) {
      $linkage = ParseLinkage $PassedTopic $TopicFile $Log
   ...
   foreach ($Section in $PassedTopic.section) {
      ParseSection $Section $TopicFile $SectionCSS $SnippetCSS $Indcrement $SeparateSnippets $InlineStats $SimpleImageScale $SmartImageScale $KeepStyles $Log
   } # foreach ($Section in $PassedTopic.section)
   ...
   foreach ($Topic in $TopicList) {
      ...
      ParseTopic $Topic $OutputFile $Sort $Prefix $Suffix $Details $Indent $SeparateSnippets $InlineStats $SimpleImageScale $SmartImageScale $Parent $TitleCSS $SubTopicCSS $SubTopicDetailsCSS $SubSectionCSS $SubSnippetCSS $Indcrement $KeepStyles $SplitTopics $Log $CSSFileName $CommandLine
   } # foreach ($Topic in $TopicList)
   ...
   switch ($PassedSnippet.type) { 
      ...
      "Picture" {
         ...
         If ($SimpleImageScale -eq 0) {
            ...
            $SimpleImageTransform = 'style="display:inline-block;'
            $SimpleImageTransform = $SimpleImageTransform + '-webkit-transform:scale(' + $SimpleImageScale + ');'
            $SimpleImageTransform = $SimpleImageTransform + '-moz-transform:scale(' + $SimpleImageScale + ');'
            $SimpleImageTransform = $SimpleImageTransform + '-o-transform:scale(' + $SimpleImageScale + ');'
            $SimpleImageTransform = $SimpleImageTransform + 'transform:scale(' + $SimpleImageScale + ');"'
            $EncodedImage = '<img ' + $SimpleImageTransform + ' src="data:image/png;base64,' + $FullImage + '">'
         }
         #>
         ...
         If ($SimpleImageScale -eq 0) {
            ...
            $EncodedImage = '<img width="' + $SimpleImageScale + '%" src="data:image/png;base64,' + $FullImage + '">'
         }
      ...
      "Smart_Image" {
         ...
         If ($SmartImageScale -eq 0) {
            ...
            $SmartImageTransform = 'style="display:inline-block;'
            $SmartImageTransform = $SmartImageTransform + '-webkit-transform:scale(' + $SmartImageScale + ');'
            $SmartImageTransform = $SmartImageTransform + '-moz-transform:scale(' + $SmartImageScale + ');'
            $SmartImageTransform = $SmartImageTransform + '-o-transform:scale(' + $SmartImageScale + ');'
            $SmartImageTransform = $SmartImageTransform + 'transform:scale(' + $SmartImageScale + ');"'
            $EncodedImage = '<img ' + $SmartImageTransform + ' src="data:image/png;base64,' + $FullImage + '">'
         }
         #>
         ...
         If ($SmartImageScale -eq 0) {
            ...
            $EncodedImage = '<img width="' + $SmartImageScale + '%" src="data:image/png;base64,' + $FullImage + '">'
         }
...
Function InsertMiscMargins ($PassedSnippet,$CSSCode,$Tag,$Log) {
   if ($PassedSnippet.contains('style="')) {
      if ($PassedSnippet.contains('margin-left:')) {
         $FirstMargin = $PassedSnippet.IndexOf('margin-left:')
         $FirstMargin = $FirstMargin + 12
         $LastMargin = $PassedSnippet.IndexOf(";",$FirstMargin)
         $MarginLength = $LastMargin - $FirstMargin
         $NoMargins = $PassedSnippet.Remove($FirstMargin,$MarginLength)


         $FirstNewMargin = $CSSCode.IndexOf('margin-left:')
         $FirstNewMargin = $FirstNewMargin + 12
         $LastNewMargin = $CSSCode.IndexOf(";",$FirstNewMargin)
         $NewMarginLength = $LastNewMargin - $FirstNewMargin
         $NewValue = $CSSCode.Substring($FirstNewMargin,$NewMarginLength)

         $NewMargins = $NoMargins.Insert($FirstMargin,$NewValue)
         $Text = $NewMargins

      } else {

         $InsertPoint = $PassedSnippet.IndexOf('style="')
         $InsertPoint = $InsertPoint + 7

         $FirstNewMargin = $CSSCode.IndexOf('margin-left:')
         $LastNewMargin = $CSSCode.IndexOf(";",$FirstNewMargin)
         $NewMarginLength = $LastNewMargin - $FirstNewMargin + 1
         $NewValue = $CSSCode.Substring($FirstNewMargin,$NewMarginLength)

         $Text = $PassedSnippet.Insert($InsertPoint,$NewValue)
      }
   } else {
      $FirstMargin = $CSSCode.IndexOf('s')
      $LastMargin = $CSSCode.IndexOf('>')
...
# Main {
   ...
   $TopicCSS = '<h3 style="margin-left:5px;">*</h3>'
   $TopicDetailsCSS = '<h4 style="margin-left:5px;">*</h4>'
   $SectionCSS = '<h5 style="margin-left:5px;">*</h5>'
   ...
   # ComposeDetails $RWExportData.Output.definition.details $RealmTitleCSS $RealmDetailsCSS
   ...
   $Parent = "/"
   foreach ($Topic in $TopicList) {
      ParseTopic $Topic $Destination $Sort $Prefix $Suffix $Details $Indent $SeparateSnippets $InlineStats $SimpleImageScale $SmartImageScale $Parent $TitleCSS $TopicCSS $TopicDetailsCSS $SectionCSS $SnippetCSS $Indcrement $KeepStyles $SplitTopics $Log $CSSFileName $CommandLine
